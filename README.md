...
*   **Navegação Híbrida:** Utilização de `Stack`, `Drawer` e `Tab` Navigators do React Navigation para uma experiência de usuário fluida e organizada.
*   **Persistência de Dados:** Implementação de um sistema de login com persistência de dados (usuário/senha) para simular a autenticação de perfil.
*   **Diferencial:** Inclusão de uma **Barra Progressiva** (linha do tempo) para monitorar o avanço do usuário nos estudos.
*   **Organização Modular:** Estrutura de código organizada em `screens`, `components`, `navigation`, `services` e `styles`, seguindo boas práticas de desenvolvimento.
